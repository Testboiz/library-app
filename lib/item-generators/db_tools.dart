...
class MySQLDBFunctions {
  ...
  static Future<bool> isMemberUnique(String name) async {
    ...
    try{
      Results nameList;
      nameList =
          await conn.query("SELECT * FROM user_account WHERE username=?",
          [name] );
  ...
  static void changeMemberInfo (
    ...
    try{
      ...
      if (username.isEmpty) {
        await conn.query("UPDATE user_account SET password = ? WHERE id_member = ?",
        [password,idMember]);
        return;
      }
      if (password.isEmpty) {
        await conn.query("UPDATE user_account SET username = ? WHERE id_member = ?",
        [username,idMember]);
        await conn.query("UPDATE member SET nama_member = ? WHERE id_member = ?",
        [username,idMember]);
        return;
      }
      await conn.query("UPDATE user_account SET username = ?, password = ? WHERE id_member = ?",
...
WHERE peminjaman.id_member = ?;""", [idMember]);
  ...
  try{
    ...
    (index) => Category(genre: dataList.toList()[index]["nama_genre"] as String,parent: parent, memberInfo: memberInfo,));
  ...
  return FutureBuilder(
    future: MySQLDBFunctions._generateCategoryButtons(parent, memberInfo: memberInfo),
    builder: ((context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) {
        return const CircularProgressIndicator();
      } else if (snapshot.hasError) {
...
MySqlConnection conn = await MySQLHandler.mySQLOpenDB();
  try{
    if (idBuku != null){
      ...
      final rawBookGenres = await conn.query("SELECT * FROM genre_buku WHERE id_buku = ? ORDER BY id_genre ASC");
      final dataList = rawDataList.toList();
...
static void pinjamBuku(String? idMember, int? idBuku) async {
  ...
  try { 
    ...
    final tabelLamaPinjam =
        await conn.query("""SELECT lama_pinjam FROM tingkat WHERE id_tingkat = 
  (SELECT id_tingkat FROM member WHERE id_member = ?);""", [idMember]);
    ...
    await conn.query("INSERT INTO peminjaman (id_member,tgl_peminjaman,tgl_kadarluasa) VALUES (?,?,?)",[
      idMember,
      todayDateString,
      deadline
    ]);
    ...
    await conn.query(
        "INSERT INTO detail_pinjaman (id_peminjaman,id_buku) VALUES (?,?)", [idPinjam,idBuku]);
  }
  finally{
    conn.close();
  }
  ...
  static Future<void> kembalikanBuku(
        int idPeminjaman, int idBuku, String idMember) async {
            ...
            try{
              // delete peminjaman
              await conn.query("DELETE FROM peminjaman WHERE id_peminjam = ?",[idPeminjaman]);
  ...
  static Future<void> kembalikanSemuaBuku (String idMember) async {
            ...
            try{
              // delete peminjaman
              final rows = await conn.query("DELETE FROM peminjaman WHERE id_member = ?",[idMember]);
              // delete detail peminjaman tidak perlu karena CASCADE
              // increment ulang buku
              await conn.query(
  ...
  MySqlConnection conn = await MySQLHandler.mySQLOpenDB();
    try{
      // fase insert buku
      await conn.query("INSERT INTO buku (judul,sinopsis,foto_sampul) VALUES (?,?,?)",[
        judul,
        sinopsis,
        fotoSampul
      ]);
      // mendapatkan pk buku
    final maxIdBukuTable = await conn.query("SELECT MAX(id_buku) AS id FROM buku");
    int idBuku = maxIdBukuTable.first["id"] as int;
    // fase insert genre buku
    for (Map m in genreMap){
      if (m["status"]== true){
        await conn.query("INSERT INTO genre_buku VALUES (?,?)",
        [idBuku,m["id_genre"]]);
      }
    }
    }
  ...
  static void editBuku(
    ...
    MySqlConnection conn = await MySQLHandler.mySQLOpenDB();
    try{
      await conn.query("UPDATE buku SET judul=?, sinopsis=?, foto_sampul=? WHERE id_buku = ?",
      ...
      for (Map m in toDelete){
        await conn.query("DELETE FROM genre_buku WHERE id_buku = ? AND id_genre = ?",
        [idBuku,m["id_genre"]]);
      }
      for (Map m in toAdd){
        await conn.query("INSERT INTO genre_buku VALUES (?,?)",
    ...
    MySqlConnection conn = await MySQLHandler.mySQLOpenDB();
    try{
      await conn.query("INSERT INTO genre (nama_genre) VALUES (?)",[namaGenre]);
    }
    finally{
      conn.close();
    }
